...
void mul_mat_vec(double** mat, double* vec, double* res, int m) {
...
void sub_vect(double* a, double* b, double* res, int m) {
    #pragma omp for schedule(dynamic, 100)
    for (int i = 0; i < m; i++) {
        res[i] = a[i] - b[i];
    }
...
void vec_to_scal(double* a, double val, double* res, int m) {
    #pragma omp for schedule(dynamic, 100)
    for (int i = 0; i < m; i++) {
        res[i] = a[i] * val;
    }
