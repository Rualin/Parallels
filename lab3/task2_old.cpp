...
using queue_elem = std::pair<size_t, std::packaged_task<T()>>;
template<typename T>
class Server {
    private:
        size_t last_task_id = 1;
        // bool is_working;
        std::queue<queue_elem<T>> tasks;
    ...
    public:
        void start() {
            ...
            while (stopVar.wait_for(u_lock, std::chrono::milliseconds(50)) == std::cv_status::timeout) {
                ...
                if (!(tasks.empty())) {
                    ...
                    // std::packaged_task<T()> func = std::move(task.second);                  //try delete move!!!
                    std::packaged_task<T()> func = task.second;
                    u_lock.unlock();
                    func();
                    // if (args.size() == 1) {
                    //     res = func(args[0]);
        ...
        // size_t add_task(std::pair<std::packaged_task<T()>, std::vector<T>> task) {
...
template<typename T>
void client_work(std::function<T()> func, std::fstream& fout, int cnt_of_arguments) {
    for (int r = 0; r < num_tasks; r++) {
        //adding task:
        std::vector<T> argument;
        for (int i = 0; i < cnt_of_arguments; i++) {
            argument.push_back(gen_random<T>());
            // argument.push_back(gen_random());
        }
        std::unique_lock<std::mutex> u_lock(mut, std::defer_lock);
        // size_t id_task = servak.add_task(std::pair<std::packaged_task<T()>, std::vector<T>>(func, argument));
        size_t id_task;
